...
Kshift = 273.15
ref_temperature = fe.Constant( Kshift + 24 )
...
class Domain:
    ...
    def define_properties(self):
        ...
        print(f"Required properties: {self.required_properties}")
        print(f"Temperature-dependent properties: {self.temperature_dependent_properties}")
        ...
        if self.mechanical_flag:
            ...
            self.lmbda = self.Emod*self.nu/(1.0+self.nu)/(1.0-2.0*self.nu)
            self.mu = self.Emod/2.0/(1.0+self.nu)
    ...
    def define_initial_conditions(self):
        ...
        self.T_old.assign(fe.interpolate(fe.Constant(ref_temperature), self.ST))
        self.T_new.assign(fe.interpolate(fe.Constant(ref_temperature), self.ST))
    ...
    def define_variational_forms(self):
        ...
        if self.phase_field_damage_flag: 
            self.ED = ( 
                -2.0*self.H_new(self.u_new)*(self.d_tst - fe.inner(self.d_trl, self.d_tst)) +\
    ...
    # Thermal deformation
    def eps_T(self):
        return self.alpha*(self.T_new - ref_temperature)*self.II
    ...
    def YieldFunction(self, sig):
        ...
        finvp = fe.conditional(fe.lt(Phi, 1.0), fe.conditional(fe.gt(Phi, 0.0), finvp_val, fe.Constant(1e5)), fe.Constant(1e5))
    ...
    # stress calculation
    def sig(self, disp):        
        return self.gdeg()*( self.lmbda*fe.tr( self.eps_e(disp) )*self.II + 2.0*self.mu*self.eps_e(disp) )
...
class Problem:
    ...
    def __init__(self,
        ...
        self.size_refinement_threshold = 0.0003
    ...
    def update_history_output(self):
        ...
        if self.thermal_flag:
            ...
            self.thermal_energy_new = fe.assemble( ( self.Part.rho*self.Part.cspec*(self.Part.T_new - ref_temperature) )*fe.dx )
